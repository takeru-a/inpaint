...
class Lafin():
    def __init__(self, config):
        ...
        if self.config.MODE == 1:
            if self.config.MODEL == 1 :
                self.train_dataset = Dataset(config,config.TRAIN_LANDMARK_IMAGE_FLIST, config.TRAIN_LANDMARK_LANDMARK_FLIST, config.TRAIN_MASK_FLIST, config.TRAIN_LABEL_IMAGE_FLIST, augment=True,training=True)
                self.val_dataset = Dataset(config,config.TEST_LANDMARK_IMAGE_FLIST,config.TEST_LANDMARK_LANDMARK_FLIST, config.TEST_MASK_FLIST, config.TEST_LABEL_IMAGE_FLIST, augment=True, training=True)
                self.sample_iterator = self.val_dataset.create_iterator(config.SAMPLE_SIZE)

            elif self.config.MODEL == 2:

                self.train_dataset = Dataset(config, config.TRAIN_INPAINT_IMAGE_FLIST, config.TRAIN_INPAINT_LANDMARK_FLIST,
                                             config.TRAIN_MASK_FLIST, config.TRAIN_LABEL_IMAGE_FLIST, augment=True, training=True)
                self.val_dataset = Dataset(config, config.VAL_INPAINT_IMAGE_FLIST, config.VAL_INPAINT_LANDMARK_FLIST,
                                           config.TEST_MASK_FLIST,config.TEST_LABEL_IMAGE_FLIST, augment=True, training=True)
                self.sample_iterator = self.val_dataset.create_iterator(config.SAMPLE_SIZE)
        ...
        if self.config.MODE == 2:
            if self.config.MODEL == 1:
                self.test_dataset = Dataset(config, config.TEST_LANDMARK_IMAGE_FLIST, config.TEST_LANDMARK_LANDMARK_FLIST, config.TEST_MASK_FLIST,config.TEST_LABEL_IMAGE_FLIST, augment=False, training=False)

            else:
                self.test_dataset = Dataset(config, config.TEST_INPAINT_IMAGE_FLIST, config.TEST_INPAINT_LANDMARK_FLIST, config.T_MASK_FLIST,config.T_LABEL_IMAGE_FLIST,
        ...
        while(keep_training):
            ...
            for items in train_loader:
                ...
                if model == 1 and self.config.AUGMENTATION_TRAIN == 1:
                    ...
                    images, landmarks, masks, masks2, images_orig, landmarks_orig, label = self.cuda(*items)
                else:
                    images, landmarks, masks, label = self.cuda(*items)
                ...
                if model == 1:
                    ...
                    landmarks_output, loss, logs, pre_label = self.landmark_model.process(images,masks,landmarks,label)
                    ...
                    if self.config.AUGMENTATION_TRAIN == 1:
                        ...
                        landmarks_output2 , loss, logs2, pre_label2 = self.landmark_model.process_aug(images_gen, masks, landmarks_orig, label)
                        self.landmark_model.backward(loss)
    ...
    def test(self):
        ...
        for items in test_loader:
            ...
            # inpaint with joint model
                ...
                output_landmark = self.landmark_model(images * (1 - masks) + masks,  masks, label)

                landmark_pred = (output_landmark[0].detach()).reshape(-1, self.config.LANDMARK_POINTS, 2).long()
                
                landmark_pred[landmark_pred >= self.config.INPUT_SIZE-1] = self.config.INPUT_SIZE-1
                landmark_last_map = self.generate_landmark_map(landmark_pred, self.config.INPUT_SIZE)
                # print(images.shape, masks.shape)
                # print(landmark_pred.shape, landmark_last_map.shape)
                ...
                for i in range(landmark_pred.shape[1]):
                    # print("ここ！！")
                    # print(int(landmark_pred[0,i,0]),landmark_pred[0,i,1])
    ...
    def sample(self, it=None):
        ...
        if model == 1:
            ...
            landmarks_pred = (self.landmark_model(images*(1-masks)+masks, masks, label))[0].reshape((-1,self.config.LANDMARK_POINTS,2)).long()
